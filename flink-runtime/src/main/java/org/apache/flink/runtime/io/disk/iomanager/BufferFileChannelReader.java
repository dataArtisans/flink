...
public class BufferFileChannelReader {
	...
	BufferFileChannelReader(FileChannel fileChannel) throws IOException {
	...
	public boolean readBufferFromFileChannel(Buffer buffer) throws IOException {
		...
		if (size > buffer.getSize()) {
			throw new IllegalStateException("Buffer is too small for data: " + buffer.getSize() + " bytes available, but " + size + " needed. This is most likely due to an serialized event, which is larger than the buffer size.");
		}
